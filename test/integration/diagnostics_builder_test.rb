...
module Alembic
  class DiagnosticsBuilderTest < ActionDispatch::IntegrationTest
    ...
    test "the builder shows a diagnostic" do
      get alembic.manage_flow_path(alembic_flows(:business_scorecard))
    ...
    test "the builder index links each diagnostic to its hub" do
      ...
      assert_select "a[href=?]", alembic.manage_flow_path(alembic_flows(:business_scorecard))
    end
    ...
    test "the edit form prefills how the diagnostic is introduced" do
      ...
      assert_select "input[name=?][value=?]", "flow[title]", "Current title"
    end

    test "updating saves how the diagnostic is introduced" do
      diagnostic = Flow::Definition.create!(slug: "editme", summary: "Old")

      patch alembic.manage_flow_path(diagnostic), params: { flow: { summary: "New summary" } }
    ...
    test "the builder index offers a form to create a diagnostic" do
      ...
      assert_select "form[action=?] input[name=?]", alembic.manage_flows_path, "flow[slug]"
    end

    test "creating a diagnostic adds it" do
      assert_difference -> { Flow::Definition.count } do
        post alembic.manage_flows_path, params: { flow: { slug: "brand-new" } }
      end
    end

    test "creating a diagnostic without a slug re-renders with the error" do
      post alembic.manage_flows_path, params: { flow: { slug: "" } }
    ...
    test "the builder index offers a remove control per diagnostic" do
      ...
      assert_select "form[action=?]", alembic.manage_flow_path(alembic_flows(:business_scorecard))
    end

    test "deleting a diagnostic removes it" do
      diagnostic = Flow::Definition.create!(slug: "deleteme")

      delete alembic.manage_flow_path(diagnostic)
