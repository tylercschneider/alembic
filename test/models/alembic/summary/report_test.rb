    ...
    class ReportTest < ActiveSupport::TestCase
      def registry
        @registry ||= Registry.new.tap do |built|
          built.register(OutputType.define(:tally) { label "Tally"; compute { |_c, run, _s| run.state.size } })
          built.register(OutputType.define(:twice) { compute { |config, _run, so_far| so_far[config["of"]] * 2 } })
        end
      end
      ...
      test "names each output it produced" do
        results = report([ { "id" => "count", "type" => "tally" } ]).results(Run.new(state: { "a" => 1 }))

        assert_equal [ "count" ], results.map(&:id)
      end

      test "carries the value an output computed" do
        results = report([ { "id" => "count", "type" => "tally" } ]).results(Run.new(state: { "a" => 1, "b" => 2 }))
      ...
      test "an output can name its own label" do
        results = report([ { "id" => "count", "type" => "tally", "label" => "How many" } ]).results(Run.new(state: {}))
      ...
      test "an output can read what an earlier one produced" do
        results = report([ { "id" => "count", "type" => "tally" },
                           { "id" => "doubled", "type" => "twice", "of" => "count" } ]).results(Run.new(state: { "a" => 1, "b" => 2 }))
      ...
      test "an output naming a type that was never registered fails clearly" do
        assert_raises UnknownOutputType do
          report([ { "id" => "x", "type" => "invented" } ]).results(Run.new(state: {}))
        end
      end

      test "a summary with no outputs produces nothing" do
        assert_empty report([]).results(Run.new(state: { "a" => 1 }))
      end
    end
  end
end
